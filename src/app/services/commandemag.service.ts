...
export class CommandemagService implements OnInit {
  ...
  getData(id: number): Observable<Object> {
    return this.http.get(HOST_URL.HOST+"/api/commMags/"+id);
  }
 
  saveOrUpdate(info: Object) {
    localStorage.setItem("Depot",JSON.stringify(info));
   return this.http.post(HOST_URL.HOST+"/api/commMags/",info);
   
  }
  
  updatedata(id: number, value: any): Observable<Object> {
    return this.http.put(HOST_URL.HOST+"/api/commMags/"+id, value);
  }

  deleteData(id: number): Observable<any> {
    return this.http.delete(HOST_URL.HOST+"/api/commMags/"+id, { responseType: 'text' });
  }

  getAll(): Observable<any> {
    return this.http.get(HOST_URL.HOST+"/api/commMags/");
  }
  getAll1(): Observable<any> {
    this.mag = localStorage.getItem('magasin');
    this.datej = this.transformDate(new Date(Date.now()));
    return this.http.get(HOST_URL.HOST+"/api/commagsd?"+"mag="+this.mag+"&"+"date="+this.datej);
  }
