...
@Component({
  ...
  styleUrls: ['./updatecommande.component.css']
})
export class UpdatecommandeComponent implements OnInit {
  public data;
  public lcoms;
  p: number = 1;
  constructor(private service:LcommandeService,private activatedRoute: ActivatedRoute,private route:Router,private toastr: ToastrService) { 
  ...
  ngOnInit(): void {
    ...
    this.service.getData1(this.data.numero)
       .subscribe(data=>{
         this.lcoms = data;
...
export class UpdatecommandeComponent implements OnInit  {
  ...
  constructor(public service:CommandeService,
    public service1:CommandefService,
  ...
  this.compteurservice.getData(this.annee).subscribe(
    response =>{
     ...
     if(this.credit==true){
      ...
      /*this.service.saveOrUpdatef(this.service.formData.value).
      subscribe( data => {
 ...
 getDocument(){
   return{
     content:[
      ...
      {
        text: (this.facture.mt / JSON.parse(localStorage.getItem('taux')).taux).toFixed(2) + ' USD',
        style:'total'
...
return{
  table:{
    ...
    body:[
      ...
      ...items.map(ed=>{
        return[ed.libart,ed.qte,(ed.pu /this.tauxusd).toFixed(2),(ed.totht /this.tauxusd).toFixed(2) ];
      })
    ],
    alignment:'center',
  }
...
getDocumentc(){
 return{
   content:[
    ...
    {
      text: ((this.facture.totttc - this.facture.mt)/JSON.parse(localStorage.getItem('taux')).taux).toFixed(2) + ' USD',
      style:'total'
    },
